...
> - **⚠️ 남은 실데이터 의존(Phase 0)**: 핸디 기준선/언더오버 기준선·배당의 베트맨 **실제 컬럼명**은
>   미확정이라 `betman.ts`의 `MARKET_FIELDS` 후보키로 추정 처리했다. 파서는 무예외라 깨져도
>   빈 결과일 뿐이며, collector 로 실응답 1회 캡처해 후보키만 교체하면 즉시 동작한다.
