...
- 단폴도 동일 슬립으로 처리(폴 1개) → 기존 `BetForm`은 슬립 패널로 흡수하거나, 단순 단폴 입력용으로 잔존.
...
## 5. 결정이 필요한 항목 (기본값 제안)
1. **폴 수 상한**: 기본 10폴. (베트맨 관례)
2. **단폴 유지 여부**: 유지(슬립 1폴). 기존 `BetForm`은 잔존 or 슬립으로 흡수 — *흡수 권장*.
3. **공동 vs 개인**: 현행대로 전부 `공동`. 개인별 분리는 별도 과제로 분리.
4. **마켓 범위**: v2는 승무패+핸디캡+언더오버 3종. 핸디언오 등은 v2.1.
