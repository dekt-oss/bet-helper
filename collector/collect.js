...
async function discoverGmTs(page) {
  ...
  try {
    ...
    await sleep(1500);
    texts.push(await page.content().catch(() => ''));
  } finally {
...
async function collectAndPost(page) {
...
// ── 주기 루프(npm start / pm2) ────────────────────────────
async function runLoop(browser) {
  let lastCollectAt = 0;
  let collectIntervalMin = cfg.baseIntervalMin;
  log(
    `주기 수집 시작: 기본 ${cfg.baseIntervalMin}분 · 경기 ${cfg.nearWindowHours}시간 전부터 ` +
      `${cfg.nearIntervalMin}분 · keep-alive ${cfg.keepaliveMin}분 (Ctrl+C 종료)`,
  );
  ...
  while (true) {
    ...
    try {
      const page = await context.newPage();
      const ok = await probeLoggedIn(page); // 이 방문 자체가 세션 keep-alive
      if (!ok) {
        warn('세션 만료/없음 → `npm run login` 으로 재로그인하세요(이후 자동 반영, pm2 재시작 불필요).');
      } else {
        // keep-alive 성공 → 갱신된 세션 쿠키를 파일에 저장(만료 지연 + 재시작 생존)
        await context.storageState({ path: cfg.sessionFile }).catch(() => {});
        const due = lastCollectAt === 0 || Date.now() - lastCollectAt >= collectIntervalMin * 60_000;
        if (due) {
          const body = await collectAndPost(page);
          lastCollectAt = Date.now();
          if (body) {
            collectIntervalMin = intervalMinForBody(body);
            log(`다음 수집까지 약 ${collectIntervalMin}분(가까운 경기 여부에 따라 조정).`);
          }
        }
      }
      await page.close().catch(() => {});
    } catch (err) {
      warn('사이클 실패(다음에 재시도):', err?.message ?? err);
    } finally {
